...
import { 
  Container,Input,Header, Item, Label, Form, 
  Text,Left,Body, Icon,Button,Content
} from 'native-base';
...
//get de ciudad
 async fechtCiudad(provinces) {
  	try{
          fetch(ipBk + '/api/address/cities/?province='+provinces,{
            method: "get",
            headers: {
  ...
  fetchData = async() =>{
      try{
          fetch(ipBk + '/api/address/provinces/',{
            method: "get",
            headers: {
                      'Accept' : 'application/json',
                      'Content-Type' :'application/json',
    ...
    async Login() {
    	...
    	if (this.state.username.length != 0 && this.state.email.length != 0 
    		...
    		&& this.state.province.length != 0 && this.state.city.length != 0 
